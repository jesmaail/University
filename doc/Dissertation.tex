...
\section{Project Management}
	...
	%Discuss risks and how they have come in, and use the gantt chart!
		...
		\begin{figure}[h]				
			...
			\centering
			\caption{Proposed (and revised) project schedule.}
	...
	\begin{figure}[h]
		\begin{tabular}{ p{2.5cm} | c | c | c | p{3cm} }
			\textbf{Risk} & \textbf{Probability} & \textbf{Impact} & \textbf{Risk score} & \textbf{Mitigation}\\ \hline
	...
	These risks can be seen in the risk table in figure \ref{fig:newRisks}, showing the probability and impact of a risk scaled from 1 to 10, with 1 being low and 10 being high. A risk score is calculated for each risk then, as the product of the probability and impact of the risk, and the risks are ordered by this risk score. 
	
	Of these risks, some did come in to play during the development of the project. \textbf{COMPLETE THIS SECTION}\textit{COVER MIGRAINES COMING ON MORE FREQUENTLY !!!!!! AS WELL AS REMOVING TOO AMBITIOUS RISK IN THE INTERIM AND THEN THE PROBLEMS FACED}.
...
\section{Analysis of problem}
	...
	%Network topology, why it is is, and a graphic of the structure
		...
		\textbf{HAVE THE STRUCTURE DIAGRAM FROM THE TECH REPORT HERE}\\
		
		In the 2013 paper from DeepMind \cite{ataridrl}, the network structure seen in figure \textbf{ref it} was used. This was later extended to the network structure seen in figure \textbf{ref it} in 2015 when published in Nature.\\
		
		\textbf{MAKE OWN STRUCTURE DIAGRAM FOR NATURE TOPOLOGY}\\
		...
		\textbf{Example image from atari, maybe have a real-life photo to compare against (if so, make it applicable e.g. of a game controller or console} 
