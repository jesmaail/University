...
def init_network():
  ...
  return ph_in, output_layer
	...
	\subsection{Training the network*}
		...
		Existing TensorFlow implementations of Deep Q-networks applied to this problem using the Arcade learning environment state that the networks require either 
		
		%Although this did not occur, existing implementations state that this process can take 3-5 days (make sure to get these as references). Explain why this was problematic late on in the project.
	
	\medskip
	
	\subsection{Testing the network*}
	%using outputs for the gradient descent to ensure it was implemented properly, no available toy box problems expect for MNIST? which is implemented differently to DQN (CNN but no DW
	\bigskip
	
\section{Conclusion}
	\subsection{Reflection on results*}
		The main deliverable of this dissertation, the C++ neural network, regrettably did not come to fruition. While prior to beginning the implementation of the network I made every effort to thoroughly learn convolutional neural networks, I still found myself needed to brush up on vital information throughout. For example the network was designed and implemented to be generated every time it was called, and initially had the weights initialised on this generation process, purely for testing. When I began work on implementing backpropagation, I attempted to change the nature of the weights in the network by externalising them, and this caused a chain reaction of errors throughout the network. This happened very late in the project timeline, so I did not have enough available time to fix it, let alone fix it and finish the backpropagation implementation.\\
		
		The network being designed to generate and then propagate when called meant that the network was slow to present it's decision, which is not ideal for the application to video games. Since working with the TensorFlow library, and implementing a convolutional neural network, I now know it would be more beneficial to have the network initialised once. Then once the network has been initialised, have available methods to propagate data forward to get a decision, and propagate data backward to train the weights of the network.
	
	\medskip

	\subsection{Suggestions for further work*}
		As was attempting with the, albeit late, attempt at the use of an existing machine learning library, I feel the best way to get working implementation for this problem would be the use of 
