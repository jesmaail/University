...
\section{Conclusion}
	\subsection{Reflection of results*}
		%Implementation suffered as a result of learning as I was going.
		%For example, the network was first built initialising the values randomly every time it was called/generated, when very late on in the timeline I corrected this, it caused horrible issues at a stage where there was not enough time to fix them.
		%With the knowledge from using the python library, I now know that the network should first have it's structure intialised only once, and then have values propagated through forwards and backwards when needed, instead of constantly re-generating the network.
	\medskip
