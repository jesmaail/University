...
rgbValue gsConvert::hextorgb(string hex){
	rgbValue rgb;
...
int gsConvert::colToGray(rgbValue colour){
...
void gsConvert::createGSTable(){
	...
	for (int i = 0; i < 16; i++){
		for (int j = 0; j < 8; j++){
			...
			rgbValue tempCol = gsConvert::hextorgb(hexToPass);
			int grayVal = gsConvert::colToGray(tempCol);
...
int gsConvert::getGrayscale(int val){
